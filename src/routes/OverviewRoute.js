...
router.get("/overview", async (req, res) => {
  try {
    ...
    if (!overview) {
      return res.status(404).send({ error: "No Overview Found" });
    }
...
router.patch("/overview", async (req, res) => {
  try {
    ...
    const updateValidation = updates.every((update) =>
    ...
    if (!updateValidation) {
      return res
        ...
        .send({
          error:
            "Invalid update. The choosen field is not available or not allowed to update",
        });
    }
